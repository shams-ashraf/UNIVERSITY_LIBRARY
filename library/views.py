...
from .models import  borrowedb,Book,userProfile
...
def borrow(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            isbnb = form.cleaned_data['isbn']
            try:
                book = Book.objects.get(isbn=isbnb)
            ...
            if book.quantity > 0:
                ...
                book.save()
                form.save()
                return JsonResponse({'success': True, 'redirect_url': '/borrowed'})
            else:
...
def borrowed(request):
    borrowed_books = borrowedb.objects.all()
